...
//JVT-W080
  ...
  ,	m_uiCurrMBX                  ( 0 )
  ,	m_uiCurrMBY                  ( 0 )
  ,	m_puiPdsInitialDelayMinus2L0 ( 0 )
  ,	m_puiPdsInitialDelayMinus2L1 ( 0 )
  , m_uiPdsBlockSize             ( 0 )
//~JVT-W080
{
...
MbEncoder::encodeMacroblock( MbDataAccess&  rcMbDataAccess,
                             IntFrame*      pcFrame,
							 IntFrame*   pcOrgFrame,//by lufeng
                             RefFrameList&  rcList0,
                             RefFrameList&  rcList1,
                             UInt           uiNumMaxIter,
                             UInt           uiIterSearchRange,
                             Double         dLambda,
                             Double&       rdCost//by lufeng
                             )
{
  ROF( m_bInitDone );
                              ...
                              Bool             bResidualPred )
{
  ...
  //JVT-R057 LA-RDO}
//JVT-W080 BUG_FIX
	RNOK ( xCheckSkipSearch( rcRefFrameList0, iRefIdxL0, rpcMbTempData, cMvPredL0.getHor(), cMvPredL0.getVer(), (UInt) LIST_0 ) );
	RNOK ( xCheckSkipSearch( rcRefFrameList1, iRefIdxL1, rpcMbTempData, cMvPredL1.getHor(), cMvPredL1.getVer(), (UInt) LIST_1 ) );
//~JVT-W080 BUG_FIX
                              ...
                              Bool              bResidualPred )
{
  ...
  if( ! pcMbDataAccessBase->getMbData().isIntra() )
  {
	...
	if(m_bLARDOEnable)
	{
		...
		{
			Int iRefIdx[2];
			iRefIdx [0]=rpcIntMbTempData->getMbMotionData(LIST_0).getRefIdx(B4x4Idx(n));
			iRefIdx [1]=rpcIntMbTempData->getMbMotionData(LIST_1).getRefIdx(B4x4Idx(n));
			IntFrame* pcRefFrame0 = ( iRefIdx [0] > 0 ? rcRefFrameList0[ iRefIdx [0] ] : NULL );
			IntFrame* pcRefFrame1 = ( iRefIdx [1] > 0 ? rcRefFrameList1[ iRefIdx [1] ] : NULL );
			Int iMvX;
			Int iMvY;
			...
			if(pcRefFrame0)
			{	 
				iMvX=rpcIntMbTempData->getMbMotionData(LIST_0).getMv(B4x4Idx(n)).getHor();
				iMvY=rpcIntMbTempData->getMbMotionData(LIST_0).getMv(B4x4Idx(n)).getVer();
				getChannelDistortion(rcMbDataAccess,*pcRefFrame0,&distortion1,iMvX,iMvY,0,0,4,4);
			}
			if(pcRefFrame1)
			{
				iMvX=rpcIntMbTempData->getMbMotionData(LIST_1).getMv(B4x4Idx(n)).getHor();
				iMvY=rpcIntMbTempData->getMbMotionData(LIST_1).getMv(B4x4Idx(n)).getVer();
				getChannelDistortion(rcMbDataAccess,*pcRefFrame1,&distortion2,iMvX,iMvY,0,0,4,4);
				if(pcRefFrame0)
					distortion1=(Int)(m_dWr0*distortion1+m_dWr0*distortion2);
				else
...
//JVT-W080 BUG_FIX
		    setPDIParameters( rcRefFrameList, iRefIdxTest, rpcMbTempData, uiDir );
//~JVT-W080 BUG_FIX
                              ...
                              MbDataAccess*     pcMbDataAccessBase )
{
  ...
  //===== BI PREDICTION =====
  if( rpcMbTempData->getMbDataAccess().getSH().getSPS().getLevelIdc() < 31 ) // bug fix: H.10.2 q) Level Constraint (NTT)
  {
...
//JVT-W080 BUG_FIX
Void MbEncoder::setPDIParameters( const RefFrameList& rcRefFrameList, Int iRefIdx, IntMbTempData*& rpcMbTempData, UInt dir )
{
	if( getPdsEnable() )
	{
		if( rcRefFrameList[iRefIdx] && rpcMbTempData->getMbDataAccess().getSH().getPoc() == rcRefFrameList[iRefIdx]->getPOC() )
		{
			UInt uiMbAddress = getCurrMBY() * getFrameWidthInMbs() + getCurrMBX();
			UInt* ppuiPdsInitialDelayMinus2;
			if( dir == 0 )
				ppuiPdsInitialDelayMinus2 = getPdsInitialDelayMinus2L0();
			else // dir == 1
				ppuiPdsInitialDelayMinus2 = getPdsInitialDelayMinus2L1();
			UInt uiPdsInitialDelay = ppuiPdsInitialDelayMinus2[0]+2; //now only index 0 is permittable
			UInt uiConstrainedMbNum = ( uiPdsInitialDelay + uiMbAddress/getPdsBlockSize() )* getPdsBlockSize() - uiMbAddress;
			m_pcMotionEstimation->setPdsEnable        ( getPdsEnable()         );
			m_pcMotionEstimation->setConstrainedMBNum ( uiConstrainedMbNum     ); 
			m_pcMotionEstimation->setFrameWidthInMbs  ( getFrameWidthInMbs()   ); 
			m_pcMotionEstimation->setCurrMBX          ( getCurrMBX()           ); 
			m_pcMotionEstimation->setCurrMBY          ( getCurrMBY()           ); 
		}
	}
}

Bool MbEncoder::SkipPDISearch( const RefFrameList& rcRefFrameList, Int iRefIdx, IntMbTempData*& rpcMbTempData, Int x, Int y, UInt dir ) //skip&direct mode all with QPel searches
{
	if( getPdsEnable() )
	{
    setPDIParameters ( rcRefFrameList, iRefIdx, rpcMbTempData, dir );
	  if( m_pcMotionEstimation->OmitPDISearch( x, y, true ) )
	    return true;
	}
	return false;
}

ErrVal MbEncoder::xCheckSkipSearch( const RefFrameList& rcRefFrameList, Int iRefIdx, IntMbTempData*& rpcMbTempData, Int x, Int y, UInt dir ) 
{
	if( getPdsEnable() )
	{
		if( rcRefFrameList[iRefIdx] && rpcMbTempData->getMbDataAccess().getSH().getPoc() == rcRefFrameList[iRefIdx]->getPOC() )
		{
			if ( SkipPDISearch( rcRefFrameList, iRefIdx, rpcMbTempData, x, y, dir ) )
